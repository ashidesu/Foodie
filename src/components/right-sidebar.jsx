...
const RightSidebar = () => {
  ...
  const [unreadCounts, setUnreadCounts] = useState({}); // State for real-time unread counts
  ...
  useEffect(() => {
    ...
    const unsubscribes = following.map(followedUser => {
      const q = query(
        collection(db, 'messages'),
        where('senderId', '==', followedUser.id),
        where('receiverId', '==', user.uid),
        where('read', '==', false)
      );
      return onSnapshot(q, (snapshot) => {
        setUnreadCounts(prev => ({ ...prev, [followedUser.id]: snapshot.size }));
      });
    });
  ...
  const handleUserClick = async (followedUser) => {
    if (user.isAnonymous) {
      alert('This feature is not available for guest users.');
      return;
    }
  ...
  return (
    <>
        ...
        <div className="profile-card">
          <div className="profile-header">
            ...
            <button
              ...
              disabled={user.isAnonymous} // Optionally disable the button visually
            >
          ...
          <div className="profile-content">
            ...
            <div className="profile-top-row">
              <div
                ...
                style={{ cursor: user.isAnonymous ? 'not-allowed' : 'pointer' }}
                title={user.isAnonymous ? 'Not available for guest users' : 'Go to Profile'}
              >
                {dbUser?.photoURL ? (
                  <img
                    src={dbUser.photoURL}
            ...
            {/* Connections below - only show if not anonymous */}
            {!user.isAnonymous && (
                ...
                {loadingFollowing ? (
                  ...
                  <ul className="following-list">
                    {following.slice(0, 5).map((followedUser) => {
                      const unreadCount = unreadCounts[followedUser.id] || 0;
                      return (
                        <li
                          key={followedUser.id}
      ...
      {selectedChatUser && (
        <ChatOverlay
          key={selectedChatUser.id}  // Added key to force remount and clear messages when chatUser changes
          currentUser={user}
